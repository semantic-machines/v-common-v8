...
pub fn resource_unavailable() -> AnyError {
    custom_error("Busy", "Resource is unavailable because it is in use by a promise")
}
...
impl JsError {
    pub fn create(js_error: Self) -> AnyError {
        js_error.into()
    }
    ...
    pub fn from_v8_exception(scope: &mut v8::HandleScope, exception: v8::Local<v8::Value>) -> Self {
        ...
        let (message, frames, stack) = if exception.is_native_error() {
            ...
            let stack: Option<v8::Local<v8::String>> = get_property(scope, exception, "stack").unwrap().try_into().ok();
        ...
        Self {
            message,
            script_resource_name: msg.get_script_resource_name(scope).and_then(|v| v8::Local::<v8::String>::try_from(v).ok()).map(|v| v.to_rust_string_lossy(scope)),
            source_line: msg.get_source_line(scope).map(|v| v.to_rust_string_lossy(scope)),
...
pub fn attach_handle_to_error(scope: &mut v8::Isolate, err: AnyError, handle: v8::Local<v8::Value>) -> AnyError {
...
impl ErrWithV8Handle {
    pub fn new(scope: &mut v8::Isolate, err: AnyError, handle: v8::Local<v8::Value>) -> Self {
